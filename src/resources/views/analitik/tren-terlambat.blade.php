...
@section('title', 'Terlambat — ' . \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y'))
...
<div class="flex items-center gap-2 text-xs text-pandora-muted mb-6">
    ...
    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-pandora-text">Terlambat</span>
</div>

{{-- Header --}}
<div class="mb-6">
    <h1 class="text-2xl font-bold text-white mb-1">Pegawai Terlambat</h1>
    <p class="text-pandora-muted text-sm">{{ $tgl->translatedFormat('l, d F Y') }} — <span class="text-pandora-gold font-medium">{{ $pegawai->count() }} pegawai</span> terlambat masuk (setelah {{ \Carbon\Carbon::parse($jamMasuk)->format('H:i') }} WITA).</p>
</div>
            ...
            <tbody class="divide-y divide-white/5">
                @foreach($perInstansi as $i => $inst)
                    <tr class="hover:bg-pandora-dark/30 transition-colors">
                        ...
                        <td class="px-4 py-2.5 text-center font-medium text-pandora-gold">{{ $inst->jumlah_terlambat }}</td>
