...
@section('title', 'Tidak Hadir — ' . \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y'))
...
<div class="flex items-center gap-2 text-xs text-pandora-muted mb-6">
    ...
    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-pandora-text">Tidak Hadir</span>
</div>

{{-- Header --}}
<div class="mb-6">
    <h1 class="text-2xl font-bold text-white mb-1">Pegawai Tidak Hadir</h1>
    <p class="text-pandora-muted text-sm">{{ $tgl->translatedFormat('l, d F Y') }} — <span class="text-pandora-danger font-medium">{{ $pegawai->count() }} pegawai</span> tidak melakukan check-in.</p>
</div>
...
<div class="bg-pandora-surface rounded-xl border border-white/5 overflow-hidden mb-6">
    <div class="px-5 py-3 border-b border-white/5">
        <h2 class="text-sm font-semibold text-pandora-text flex items-center gap-2">
            ...
            Instansi dengan Ketidakhadiran Terbanyak
        </h2>
    </div>
                ...
                @forelse($pegawai as $p)
                    <tr class="hover:bg-pandora-dark/30 transition-colors">
                        ...
                        <td class="px-4 py-2.5 text-center">
                            @if($p->is_manual)
                                <span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-pandora-muted/20 text-pandora-muted">Presensi Manual</span>
                            @elseif($p->kategori_ijin)
                                <span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-pandora-accent/20 text-pandora-accent">{{ $p->kategori_ijin }}</span>
                            @else
                                <span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-pandora-danger/20 text-pandora-danger">Tanpa Keterangan</span>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-6 text-center text-pandora-muted">Semua pegawai hadir</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
