...
class FeatureKehadiranHarian(Base):
    ...
    jarak_dari_geofence_berangkat = Column(Numeric(12, 4))
    jarak_dari_geofence_pulang = Column(Numeric(12, 4))
    geofence_match_flag = Column(String(20))
    aplikasi_fake_gps_terdeteksi = Column(Boolean, default=False)
    sumber_presensi = Column(String(15), default="unknown")
...
class AnomalyFlag(Base):
    ...
    id = Column(BigInteger, primary_key=True)
    id_pegawai = Column(BigInteger, index=True, nullable=False)
    nama_snapshot = Column(String(150))
    nip_snapshot = Column(String(30))
    nama_unit_snapshot = Column(String(200))
    tanggal = Column(Date, index=True, nullable=False)
